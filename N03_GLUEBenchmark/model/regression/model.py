...
class RegressionModel(nn.Module):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        if args.model_type == 'cnn':
            ...
            self.regressor = nn.Sequential(
                ...
                nn.GELU(),
                nn.Linear(args.hidden_size, 1) # Regression - Single output
            )
            ...
            self.regressor = nn.Sequential(
                nn.Linear(self.linear_size, self.hidden_size),
                nn.Dropout(self.args.dropout_rate),
                nn.GELU(),
                nn.Linear(self.hidden_size, 1) # Regression - Single output
            )
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, token_type_ids: torch.Tensor) -> torch.Tensor:
        if self.args.model_type == 'cnn':
            ...
            regression_pred = self.regressor(conv_output) # (batch_size, 1)
            ...
            regression_pred = self.regressor(rnn_hidden) # (batch_size, 1)
            ...
            reg_output = transformer_output.mean(dim=1) # (batch_size, embed_size) - Global average pooling

            regression_pred = self.regressor(reg_output) # (batch_size, 1)
        else: # Huggingface models
            model_output = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, return_dict=True)
            reg_output = model_output.last_hidden_state[:, 0, :] # (batch_size, hidden_size)
            regression_pred = self.regressor(reg_output) # (batch_size, 1)
