...
def training(args: argparse.Namespace) -> None:
    ...
    reg_loss = nn.MSELoss()
    write_log(logger, f"Loss function: {reg_loss}")
    ...
    if args.job == 'resume_training':
        ...
        if args.use_wandb:
            ...
            wandb.watch(models=model, criterion=reg_loss, log='all', log_freq=10)
        del checkpoint
    ...
    for epoch_idx in range(start_epoch, args.num_epochs):
        ...
        for iter_idx, data_dicts in enumerate(tqdm(dataloader_dict['train'], total=len(dataloader_dict['train']), desc=f'Training - Epoch [{epoch_idx}/{args.num_epochs}]', position=0, leave=True)):
            ...
            batch_loss_reg = reg_loss(regression_pred.squeeze(), labels.squeeze())

            # Train - Backward pass
            optimizer.zero_grad()
            batch_loss_reg.backward()
            ...
            if iter_idx % args.log_freq == 0 or iter_idx == len(dataloader_dict['train']) - 1:
                write_log(logger, f"TRAIN - Epoch [{epoch_idx}/{args.num_epochs}] - Iter [{iter_idx}/{len(dataloader_dict['train'])}] - Loss: {batch_loss_reg.item():.4f}")
            if args.use_tensorboard:
        ...
        if args.use_tensorboard:
            writer.add_scalar('TRAIN/Loss', train_loss_reg / len(dataloader_dict['train']), epoch_idx)
        ...
        for iter_idx, data_dicts in enumerate(tqdm(dataloader_dict['valid'], total=len(dataloader_dict['valid']), desc=f'Validating - Epoch [{epoch_idx}/{args.num_epochs}]', position=0, leave=True)):
            ...
            # Valid - Logging
            valid_loss_reg += batch_loss_reg.item()

            if iter_idx % args.log_freq == 0 or iter_idx == len(dataloader_dict['valid']) - 1:
                write_log(logger, f"VALID - Epoch [{epoch_idx}/{args.num_epochs}] - Iter [{iter_idx}/{len(dataloader_dict['valid'])}] - Loss: {batch_loss_reg.item():.4f}")

        # Valid - Call scheduler
        if args.scheduler == 'LambdaLR':
            scheduler.step()
        elif args.scheduler == 'ReduceLROnPlateau':
            scheduler.step(valid_loss_reg)

        # Valid - Check loss & save model
        valid_loss_reg /= len(dataloader_dict['valid'])

        if args.optimize_objective == 'loss':
            valid_objective_value = valid_loss_reg
            valid_objective_value = -1 * valid_objective_value # Loss is minimized, but we want to maximize the objective value
        else:
            raise NotImplementedError(f"Regression tasks do not support {args.optimize_objective} as the optimization objective")
        ...
        if args.use_tensorboard:
            writer.add_scalar('VALID/Loss', valid_loss_reg, epoch_idx)
        if args.use_wandb:
            wandb.log({'TRAIN/Epoch_Loss': train_loss_reg / len(dataloader_dict['train']),
                       'VALID/Epoch_Loss': valid_loss_reg,
                       'Epoch_Index': epoch_idx})
            wandb.alert(
                title='Epoch End',
                text=f"VALID - Epoch {epoch_idx} - Loss: {valid_loss_reg:.4f}",
